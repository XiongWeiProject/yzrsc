...
import com.commodity.scw.http.excutor.PoolExecutor;
import com.commodity.scw.manager.NetworkManager;
...
public class HttpManager extends BaseHttpManager {
	...
	private HttpParam mHttpParam;// 通信参数
	 ...
	 *            URL
	 */
	public HttpManager(int tag,String url) {
	 ...
	 *            URL
	 */
	public HttpManager(int tag,HttpMothed mothed, String url) {
	 ...
	 *            传入对象
	 */
	public HttpManager(int tag, HttpMothed mothed, String url, Map<String, String> obj) {
	 ...
	 */
	public HttpManager(int tag, HttpMothed mothed, String url, Map<String, String> obj,
					   IRequestListener listener, boolean hasSecurityCertificate) {
	 ...
	 */
	public HttpManager(int tag, HttpMothed mothed, String url, Map<String, String> obj,
					   IRequestListener listener, List<UploadFile> fileList) {
	...
	public HttpManager(int tag, HttpMothed mothed, String url, Map<String, String> obj,
		...
		try {
			...
			mHttpParam = new HttpParam(mothed, url, obj,
	 ...
	 *            监听实例
	 */
	public HttpManager(int tag, String url, IRequestListener listener) {
		try {
			mClient = HttpClientFactory.instance();
	...
	private void execute() {
		HttpMothed mothed = HttpMothed.POST;// 默认post请求
		if (mHttpParam != null) {
	...
	private void switchHttpChannel(HttpChannel channel, HttpMothed mothed) {
		...
		// mHttpUrlConnectionExecutor = new HttpUrlConnectionExecutor();
		// switch (mothed) {
		// case GET:
		// mClient.get(mHttpParam, mHttpResponse);
		// break;
