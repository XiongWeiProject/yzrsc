package com.commodity.yzrsc.http.quickimageloder;
...
import com.commodity.yzrsc.http.quickimageloder.ImageSizeUtil.ImageSize;
...
public class QuickImageLoader {
    private static QuickImageLoader mInstance;
    ...
    private static final String TAG = "QuickImageLoader";

    public enum Type {
        FIFO, LIFO;
    }

    private QuickImageLoader(int threadCount, Type type) {
    ...
    public static QuickImageLoader getInstance() {
        if (mInstance == null) {
            synchronized (QuickImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new QuickImageLoader(DEAFULT_THREAD_COUNT, Type.LIFO);
                }
            }
        }
        return mInstance;
    ...
    public static void create(Application context) {
        if (mInstance == null) {
            mInstance = new QuickImageLoader(3, Type.LIFO);
        }
    }

    public static QuickImageLoader getInstance(int threadCount, Type type) {
        if (mInstance == null) {
            synchronized (QuickImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new QuickImageLoader(threadCount, type);
                }
            }
        }
        return mInstance;
